 ...
 *
 */
class BillMateAPI extends StdClass{
 	var $SERVER = "1.3.5";
 	var $CLIENT = "";
 	...
 	function BillMateAPI($eid,$key,$ssl=true,$debug=false,$test=false){
 		$this->encoding = 2;
 		$this->eid = $eid;
 		$this->key = $key;
 		$this->ssl = $ssl;
    ...
    protected function call($method, $array) {
        ...
        if ($status !== 0){
					billmate_log_data(array('request'=>$array,'response'=>$xmlrpcresp->faultString()), $this->eid, $method.'-error');
        ...
        $result = php_xmlrpc_decode($xmlrpcresp->value());
				...
				$string_response = php_xmlrpc_decode( $xmlrpcresp->value());
        billmate_log_data(array('request'=> $array,'response' => $string_response), $this->eid, $method);
